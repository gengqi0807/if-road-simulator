...
function createStepRecord(stepIndex, option) {
  return {
    stepIndex,
  ...
  return (
    <main className="comparison-page">
      ...
      {branches.length < 2 ? <section className="timeline-empty comparison-empty">请先在推演页完成至少两条路径</section> : <>
        ...
        <div className="comparison-layout">
          <section className="comparison-table-card"><div className="comparison-table-wrap"><table className="comparison-table"><thead><tr><th>对比指标</th>{selectedBranches.map((branch) => <th key={branch.id}>{branch.name}<small>已完成</small></th>)}</tr></thead><tbody>{rows.map(([label, getter]) => <tr key={label}><th>{label}</th>{selectedBranches.map((branch) => <td key={branch.id}>{getter(branch)}</td>)}</tr>)}</tbody></table></div></section>
...
function SimulationPage({ branch, initialStepIndex = null, onBranchUpdate, onOutcome }) {
  ...
  const currentOptions = currentStepIndex === 1
    ? firstStepOptions
    : currentStepIndex === 2
  ...
  const handleConfirm = () => {
    ...
    setIsAnalyzing(true);
    window.setTimeout(() => {
      const nextRecord = createStepRecord(currentStepIndex, selectedOption);
      ...
      setAnalysis({ ...demoAnalysis, title: selectedOption.key === 'B' && currentStepIndex === 1 ? demoAnalysis.title : `你选了${selectedOption.text}` });
      setIsAnalyzing(false);
      if (currentStepIndex === totalDecisionSteps) {
        onBranchUpdate({ ...nextBranch, status: 'ended', outcome: createOutcome(nextBranch) });
        onOutcome({ ...nextBranch, status: 'ended', outcome: createOutcome(nextBranch) });
      } else {
  ...
  return (
    <main className="simulation-shell">
      ...
      <div className="simulation-content">
        ...
        <ChoiceCard step={currentStep} selectedKey={selectedKey} onChoose={(option) => !isAnalyzing && setSelectedKey(option.key)} disabled={isAnalyzing} />
        <button className="confirm-choice-button" type="button" disabled={!selectedKey || isAnalyzing} onClick={handleConfirm}>
...
function App() {
  ...
  const startSession = (payload) => {
    ...
    const session = { id: sessionId, goal: payload.goal.trim(), createdAt: new Date().toISOString(), branches: [branch], activeBranchId: branch.id, branchCounter: 1 };
  ...
  const handleCreateBranch = (stepIndex) => {
    if (!currentSession || !currentBranch) return;
    const nextCounter = currentSession.branchCounter + 1;
    const branch = createBranch(`branch-${nextCounter}`, getBranchName(nextCounter), currentBranch.id, stepIndex, currentBranch.steps.filter((step) => step.stepIndex <= stepIndex));
  ...
  if (route.stage === 'report') return <ReportPage branch={currentBranch} onBack={() => navigate(`/outcome/${currentSession.id}/${currentBranch.id}`, { stage: 'outcome', sessionId: currentSession.id, branchId: currentBranch.id })} onRestart={() => createNewBranch(currentSession.id)} />;
  return null;
}
